...
This document covers the complete pipbounce API surface: the HTTP control server exposed by the daemon, the Chrome extension architecture, the full configuration model, the macOS permission requirements, and common troubleshooting scenarios. It is intended for developers integrating with pipbounce or building tooling on top of it.
...
Navigate to **System Settings → Privacy & Security** and add `pipbounce` (located at `~/.pipbounce/pipbounce.app` after install) to each of the three categories:
...
|---------|-------------|-----------|
| PiP window does not move away from cursor | Accessibility permission not granted | System Settings → Privacy & Security → Accessibility → add `pipbounce` |
| Status bar shows "Offline" immediately after install | Daemon not running or launchd plist not loaded | Run `launchctl load ~/Library/LaunchAgents/com.pipbounce.daemon.plist` or re-run `install.sh` |
| Hotkey does not respond | Input Monitoring permission not granted | System Settings → Privacy & Security → Input Monitoring → add `pipbounce` |
| `GET /status` returns `pipActive: false` even with a PiP window open | Screen Recording permission not granted | System Settings → Privacy & Security → Screen Recording → add `pipbounce` |
...
| Alt+P hotkey does not enter PiP | Extension shortcut may be overridden by another extension | Navigate to `chrome://extensions/shortcuts` and reassign the `toggle-pip` command for pipbounce |
