...
2. **macOS daemon** (`~/.pipbounce/pipbounce`) -- A Swift process that polls the mouse position at 30fps via `CGEvent`, locates the PiP window through the macOS Accessibility API (`AXUIElement`), and decides whether to dodge or allow interaction based on the mouse's entry angle.
...
This compiles `pipbounce.swift` into `~/.pipbounce/pipbounce` and generates extension icons.
...
2. Add `~/.pipbounce/pipbounce` (or the terminal application you run it from)
...
```
daemon/pipbounce.swift        macOS daemon: mouse tracking, window movement, HTTP API
...
|                     |   GET /status                           |                    |
|  popup.js           |   POST /settings {json}                 |  pipbounce.swift    |
...
A PID lock file at `~/.pipbounce/pipbounce.pid` ensures only one daemon instance runs. Starting a new instance sends `SIGTERM` to the old one.
...
Start the daemon manually with `~/.pipbounce/pipbounce`. It must be running for settings sync and status display to work. Dodge behavior depends entirely on the daemon; the extension only handles PiP activation.
