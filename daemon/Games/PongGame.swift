...
class PongGame: GameBase {
    ...
    override func onStop() {
        ...
        let pw = playerPaddle, aw = aiPaddle, tw = trailWindow
        let cleanup = {
            pw?.orderOut(nil)
            aw?.orderOut(nil)
            tw?.orderOut(nil)
    ...
    override func gameTick() {
        ...
        if velocity.x > 0 && ballPos.x + size.width > midX {
        ...
        updatePaddlePositions(playerY: pY, screen: screen, shakeOffset: shakeOff)
        syncBorder(around: bounds)
        updateTrail(ballPos: ballPos, size: size, shakeOffset: shakeOff)
        if scoreChanged { updateScore(); scoreChanged = false }
        CATransaction.commit()
    }
    ...
    private func updateTrail(ballPos: CGPoint, size: CGSize, shakeOffset: CGPoint) {
        ...
        // Store position every 2 frames for spacing
        trailFrameCounter += 1
        if trailFrameCounter >= 2 {
            trailFrameCounter = 0
            trailPositions.insert(CGPoint(x: ballPos.x + size.width / 2,
                                           y: ballPos.y + size.height / 2), at: 0)
            if trailPositions.count > trailCount + 2 {
                trailPositions.removeLast()
            }
        }

        let dotSize: CGFloat = max(size.width, size.height) * 0.4
        let color = glowCGColor()

        for i in 0..<trailLayers.count {
            let posIdx = i + 1  // offset by 1 so trail lags behind ball
    ...
    private func createOverlays(screen: CGRect) {
        ...
        // Score overlay positioned relative to screen geometry
        ...
        // Trail window covering play area
    ...
    private func updatePaddlePositions(playerY: CGFloat, screen: CGRect, shakeOffset: CGPoint = .zero) {
        let h = screenH

        playerPaddle?.setFrame(
            NSRect(x: screen.minX + paddleMargin + shakeOffset.x,
                   y: h - playerY - paddleHeight + shakeOffset.y,
